...
export const MUTATIONS = [
     ...
     1回目: verify-admin.mjs（`docs/ops/proof-of-red.md` の「## F4 を閉じる範囲」）。 */
  {
    ...
    why: '**お店の人に「管理」の入口が出なくなる**——カルテ画面から管理画面へ行く道が1つも無くなる（マスターが実際に詰まった形）',
  ...
  {
    id: 'admin-owner-gate-off',
    why: '**飼い主にも管理画面が開ける**——お客様が他人の顧客データを消せてしまう',
    file: 'backend/js/supabase-admin.js',
    find: '    if ((session.memberships || []).length === 0) {',
    replace: '    if (false) {',
    extra: null,
  ...
  },
  {
    ...
    file: 'src/index.html',
    find: '<textarea id="editor-trimmer-letter" data-field="staff-note" class="trimmer-textarea" rows="4" placeholder="今日の様子を、飼い主さんへ一言。" oninput="App.updateCompletionStatus()"></textarea>',
    replace: '<textarea id="editor-trimmer-letter" data-field="staff-note" class="trimmer-textarea" rows="4" placeholder="今日の様子を、飼い主さんへ一言。" oninput="App.updateCompletionStatus()">今月もとってもお利口にしていました。</textarea>',
    extra: null,
    scripts: ['verify-edit.mjs'],
  },
