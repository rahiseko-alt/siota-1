...
| **D-15** | **ルールの正は `AGENTS.md` ひとつだけ。** 特定の AI だけが読む設定（`.claude/` `.codex/` `*.claude.json` `CLAUDE.md` の中身）にルールを書かない。このリポジトリは複数の AI が共同で触るので、2箇所にあると必ず食い違う | `.gitignore` |
| **D-16** | いまは**整理整頓プラン（F1→F2→F3）の期間中**。現在のフェーズは `docs/ops/phase`、計画の正は `docs/ops/plan.md`。フェーズの範囲外を触らない・新しい物を足さない・見つけた問題はあと回し（下記） | `npm run guard` |
