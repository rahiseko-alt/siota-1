...
| **`photo-trim-action.jpg`** | **残存・使用中**（下記） |
...
### まだ残っている問題 — `photo-trim-action.jpg`
...
- [ ] **`photo-trim-action.jpg`（最優先）**: ログイン画面で使用中・`UNVERIFIED`・
      ログイン前の誰にでも見える。マスターが撮影したものか確認する
...
- [ ] 使用中の `UNVERIFIED`（`teeth-diagram.jpg` / `body-side.png` / `icon-*.png` 5件）の
      ...
      外部公開物であることに変わりはない
- [ ] `nail-diagram.png` 6.1MB の圧縮を検討する。ただし状態判断に使う図なので、
      画質を落とすかは用途の確認が先
