...
1. **3つ目の毒**: **配信物を壊す**（`dist/` を空にする等）← 残る15件を判定する唯一の道
2. **入口で死ぬ139件**をどうするか。土台が空だと「0. 検査用の犬を登録できた」で
   落ちて先へ進めない。**犬は居るがカルテが空**のような**中間の毒**が要る
3. **マスター判断待ち**: `check()` に行番号か id を印字させるか（182か所の書き換え）
4. `docs/watch.md` の W-2〜W-8
5. `docs/deferred.md` の残りを閉じる
