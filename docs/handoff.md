...
## 0. 最新（2026-08-25）— **F3 の地雷を11件中10件まで潰し、検査を CI へ移した**

### いま実行できるものと、その結果（すべて実測）

| 命令 | EXIT | 中身 |
|---|---|---|
| `npm run build` | **0** | 35件（`my.html` と `backend/js` + vendor バンドルを含む） |
| `npm run check` | **0** | guard / 隔離 src+dist（**条件A のみ**）/ parity / design / **削除順序** / **繋ぎ方** |
| `npm test` | **0** | **100件**（session 開始時は 64件） |
| `npm run walk` | **0** | ①〜⑥ 5コマ |
| `npm run walk mistakes` | **0** | 8コマ |
| **GitHub Actions** | **緑** | `check`（約20秒）と `verify`（実 Supabase・約4分）の2 job |

**`npm run verify:*` は CI で走る。** 手元に Docker が無くても、PR ごとに自動で走る。

### このセッションでやったこと

**F3 の②バッドシナリオを「死ぬ順」に11件出し、10件を解決した**（1件につき赤→緑→戻して赤の3出力・`docs/ops/solved-F3.md`）。

| 段 | 項目 |
|---|---|
| I 取り返しがつかない | ~~#1 偽の手紙~~ ／ ~~#2 消せない写真~~ |
| II 黙って失う | ~~#3 しこりの印~~ ／ ~~#4 写真の無言欠落~~ ／ ~~#5 SQL が緑~~ |
| III 気づけない | **#6 届く中身の検査**（1/9）／ ~~#7 ⑥の器~~ |
| IV 進めない | ~~#8 walk~~ ／ ~~#9 隔離検査~~ ／ ~~#10 規格違い~~ ／ ~~#11 関所~~ |

**マスターの問い「根本的な解決はなんだ？」から計画を練り直した**（`docs/ops/plan.md`）。
「Docker が無い」は症状で、根っこは**検査が一台の机の上でしか動かない形**だった。
接続先を差し替え可能にし、**CI に移した**（`D-20260825-44`）。ランナーに Docker が在るので、
**マスターの Windows にもエージェントのコンテナにも Docker は要らない**。

### 次にやること — F3 4-1 の残り

**`/edit` は直った**（`502` → 正UI を配信・CI で7項目 PASS）。残りはこれ:

1. **描画係の口を合わせる。** `TrimmerSupabaseStaff.boot(PonchiApp)` は**描画係を引数で取る**
   （自動起動しない）。`PonchiApp` は依存ではなく**差し替え可能な口**なので、
   正UI 側に `show(screen, data)` を用意して渡す。**当初の想定より軽い**
2. **残り8本の `verify:*` を戻す**（`#6` の残り）。`/edit` が通るようになったので塞がりは取れた
3. **繋ぐ前に片づけるもの**: `magazine-view.js:322` の `img.src = ''`（`deferred` #16）／
   ⑥が読む **14キー**に対し出す側が **1キー**しか無い（`F-20260821-12/-13` の型）

### 私がこのセッションで出した失敗（重要）

- **`F-20260825-35` / `-36`: 同じ型を2回。** 「**期待する成功の形を、実際の仕組みに合わせて書かずに検査を書いた**」。
  1回目は `status !== 404` を合格条件にし、マイグレーション未適用でも PASS する検査を作った。
  2回目は `globalThis.App` を見て落ち、**製品ではなく検査が間違っていた**——しかも
  その事実は `#10` の調査中に自分で実ブラウザで確かめていたのに、使わなかった。
  **`AGENTS.md` への昇格候補**（`ルール昇格原則` の「複数回発生した」に該当）。**マスターの判断待ち**
- **どちらも CI が無ければ気づけなかった。** 手元では走らせられない検査だから

### マスター判断待ち

**鍵3種のローテーション**（`D-20260824-31` / `D-20260825-41`・**実顧客データ投入前に必須**）／
上記の**ルール昇格の可否**／`docs/deferred.md` **全22件**（`#16` `#20` `#21` `#22` は今回追加）／
担当メッセージの既定文2か所 ／ `01〜04` タブ ／ その他は `plan.md` 第6章。
