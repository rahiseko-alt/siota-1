...
## 0-A. いちばん新しい（2026-08-27）— **管理者画面が CI で全項目通った（PR #17・master 未マージ）**

**この下の「## 0」は master（`d48811b`）の状態。** いま master に**入っていない**ものが1本ある。

- **[PR #17](https://github.com/rahiseko-alt/siota-1/pull/17)（`claude/checkin-62qojx`・CI 3 job とも緑）** — PR #16 の続き（#16 は閉じてよい）
...
1. **PR #17 を master に入れる**（`npm run check`・`npm test`・CI はすべて緑。マージはマスター判断）
2. そのあとは下の「次に効いてくること」——**鍵3種のローテーション → デプロイ → 本番のダミー掃除**の順
