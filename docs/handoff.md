...
1. **台帳の鍵を直す**（動的な名前・同名の複数箇所を区別できるようにする）。
   直せば `verify-xss` の8件が移せる
2. **毒の種類を増やす** ← 164件を埋める唯一の道。いま在るのは「データが空」だけ。
   要るのは少なくとも: **認証を拒む毒**（`verify-stack` の「実ログインできる」用）／
   **犬は居るがカルテが空の毒**（入口で死ぬ139件を先へ進ませる）
