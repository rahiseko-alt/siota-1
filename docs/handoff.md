...
4. **削除の入口がどの画面にも無い**（`#25`・サーバ側は実装済み。置き場所がマスター判断待ち）
   → **マスター判断待ちのまま。** 意匠モックに削除のボタンが無く、思いつきで足すのは `D-18` 偽-7
5. 過去カルテを開く導線が無い（`#23`・**マスター判断待ち**）／届く日付が下書き作成日のまま（`#29`）
   → **`#29` は原因を特定した（`deferred` #33）。** 確定時の PATCH が `{ data }` しか送らず、
   `updateReportSchema` は `.strict()` で `data` のみ＝**日付を送る口が構造的に無い**。
   DB も `grant update (data)` だけで **トリマーのトークンでは `report_date` を書き換えられない**。
   walk が `2026.07.16` を撮ったのは `seed.sql` の draft fixture を再開して確定したため。
   **直すには「施術日をどう決めるか」の規則をマスターが決める必要がある**（下書き開始日 / 確定日 / ④に入力欄）
