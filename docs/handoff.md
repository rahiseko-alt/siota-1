...
## 0. 最新（2026-08-26）— **消えていた検査9本を全部戻し、④⑤⑥を結線した**
...
|---|---|---|
| `npm run build` | **0** | 35件 |
| `npm run check` | **0** | guard / 隔離（条件A）/ parity / design / 削除順序 / 繋ぎ方 / **検査台帳** / **キー対応** |
| `npm test` | **0** | **112件** |
| **GitHub Actions** | **緑** | `check`（約20秒）と `verify`（実 Supabase・約6分） |
...
**`bad-scenarios-F3` の未解決を 0 件にした**（`#6` を実体の9本に割り直し、`#12`〜`#19` を全部解決）。
`gate.mjs --end` は「**F3 は閉じてよい状態です**」と言う。**ただし閉じていない**（下記）。

**4-1 の結線**: ②一覧・③選択・④保存確定・⑤確認・⑥顧客ページ。
`App.show(screen, data)` を正UI に用意し、`TrimmerSupabaseStaff.boot(App)` に渡した。
判定は「backend が居るか」だけで URL は見ない——静的配信の `/`（F2 の walk 経路）は
これまでどおり仮データで動く。

**検査が見つけた製品の欠陥（全部この作業中に出た）**:

| | 中身 |
|---|---|
| `weights` に `ym` が無く**体重が「未記録」**だった | `roundtrip` |
| `trimming` を `length`/`style` で出し**どこにも表示されなかった** | `roundtrip` |
| 犬体図の印が **`data:,`（中身の無い画像）** で保存されていた（`F-20260825-38`） | `roundtrip` |
| `/edit/p/{id}` から「02」を押すと**一覧が空**で犬を選び直せない（`F-20260825-39`） | `m6` |
| 書いた直後に画面を移ると**下書きが保存されていない** | `m6` |
| 空の写真スロットが**ページURL**を指していた（`deferred` #16） | 結線前に片づけ |

### 次にやること — **F3 4-2（フェーズを閉じるのはこの後）**

**`gate.mjs --end` が緑でも、F3 の完了条件は満たしていない。**
完了条件は「D-14 の2問に、**実データのスクリーンショットだけで**答える」（`plan.md` 4-2）。
いま `npm run walk` は**仮データの `/`** を撮っている。実データに向ける作業が要る。
**9本が緑になっただけではフェーズは閉じられない。**

### 私がこのセッションで出した失敗

- **`F-20260825-35`/`-36` の型を4回出した**（歯のボタンの表示と保存値の違い／招待の航法／
  `m6` の下書き上書き／`data.pet` の描画条件）。**どれも CI が止めたので間違った緑は出ていない**が、
  「書く前に仕組みを測る」を機械にできていない。**`AGENTS.md` への昇格候補**（マスター判断待ち）
- **`F-20260825-37`**: 「戻すと赤」を、直したものとは別の層で採ろうとした（`solved.mjs` が止めた）
- **見ていない結果（`19/19 PASS`）を記録に書いた**。偽-6 そのもので、実際に見た出力に戻した
- **合格した行に失敗時の文言を出していた**（`PASS … ★ 文例が出ている`）。9本とも直した

### マスター判断待ち

**鍵3種のローテーション**（実顧客データ投入前に必須）／
**④の入力欄の既定文**（`deferred` #13・`verify:edit` が毎回1件出す）／
**削除の入口をどこに置くか**（`#25`・`verify:delete` が毎回1件出す）／
**歯の表示と保存値のずれ**（`#24`・`verify:roundtrip` が毎回1件出す）／
**`F-20260825-35`/`-36` のルール昇格**／`docs/deferred.md` **全27件**／その他は `plan.md` 第6章。
