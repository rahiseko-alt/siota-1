...
1. **同姓同名を区別できない**（`deferred` #36・マスター指摘）。削除の一覧が名前しか出さず、
   確認の入力も「名前を打つ」なので、**同名の相手には確認が働かない**。DB 側は UUID なので問題ない
2. **バックアップが無い**（`deferred` #35・`D-20260827-46`）。クライアントの意向は
   「納品後、使いながら考える」。CSV で取り出す準備だけ置いてある（`runbook.md` 2-B）
