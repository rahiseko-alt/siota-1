...
## 0. 最新（2026-08-25）— F1 完了。次は F2

**整理整頓プランの F1（UI とバックエンドの隔離）を閉じた。**
`docs/ops/bad-scenarios-F1.md` の「該当した」**8件すべてを解決**し、1件につき3出力
（赤 → 緑 → 戻して赤）を `docs/ops/solved-F1.md` に貼った。

### いま実行できるものと、その結果（すべて実測・3回連続で同じ）

| 命令 | EXIT | 中身 |
|---|---|---|
| `npm run build` | **0** | 34件 |
| `npm run check` | **0** | guard / **隔離 src** / **隔離 dist** / src↔dist parity / design 隔離 の**5本** |
| `npm test` | **0** | **64件**（unit 5 / schema 47 / migration 6 / **backend-import 6**） |
| `node scripts/guard/solved.mjs F1` | **0** | 8件すべてが3出力で裏づけ済み |
| `node scripts/guard/gate.mjs --end` | **0** | F1 を閉じてよい状態 |
| `npm run walk` | **1** | **④→⑤へ進めない**（下記） |
...
| # | 何が起きていたか | どう直したか |
|---|---|---|
| 1 | `npm run check` に隔離を見る検査が **0 本** | `scripts/guard/isolation.mjs` を新設し `check` に組込（0本 → 1本） |
| 2 | 検査が、画面の本体 `src/index.html`（2,340行）を走査していなかった | 走査対象を「`index.html` から実際に辿れたファイル」にした。起点なので必ず含まれる |
| 4 | 検査を書いても、赤くなるところを一度も見ていなかった | **5つの違反型すべて**（backend 参照 / Supabase SDK / `/api/` / 外部URL / `fetch` 等）で EXIT 1 を実測 |
| 6 | 「UI 以外が無い」の基準が無かった | 基準を「`src/index.html` から辿り着けるか」1本に決めた。未到達 **11件**を列挙し、**1件も消さず** `docs/deferred.md` に番号付きで登録 |
| 7 | `backend/js/magazine-view.js` を消したまま、呼び出し元が生きていた | `git show '6685df5^:src/js/magazine-view.js'` で復元（579行・書き直していない） |
| 8 | `npm test` が backend を見ておらず、**壊れていても緑**だった | `test/backend-import.test.mjs` を新設・配線。1本ずつ壊して測ると**見逃し 4本 → 0本** |
| 9 | `src-dist-guard.config.json` が実在しない `src/design-samples` を指し、検査が**黙って飛ばして合格**にしていた | 死んだ項を削除し、検査を「**実在しない場所を指したら EXIT 1**」に変えた |
| 10 | まっさらな環境では `npm run check` が意味不明に落ちた | **判定は緩めず**（EXIT 1 のまま）、「まだ組み立てていない・`npm ci` → `build` → `check`」と案内するようにした |

作業中に**検査自身の不具合を3件**出して直した（`F-20260825-31`）。とくに `solved.mjs` は、
`m` フラグ下の `$` が各行末に当たるせいで**本文を1行目までしか読めておらず**、正しく書いた記録を
8件すべて「書式違反」と報告していた。直したあと、**検査が緩くなっていないこと**を
3種の欠落（③削除 / 種別を回避にして番号なし / ①の出力を空）で実測して確かめた。
...
- 原因は **463px 問題**（④の編集領域が横 463px 必要・iPhone 13 は 390px）。**F2 の作業**
- **直し方はマスター判断**: (a) 編集領域だけ横スクロール容認 / (b) モックのレイアウト意図に沿った縮小

> はじめ `walk` はブラウザが見つからず落ちていた（環境の chromium とバージョン違い）。
> `WALK_CHROMIUM=/path/to/chrome npm run walk` で実行ファイルを渡せるようにした——
> **ブラウザが無いだけの失敗と、アプリの失敗を混同しないため。**
...
| `docs/deferred.md` の**あと回し** | **10 件**（今回 +8） |
| `docs/failures.md` | 全 **32件** / OPEN **1件**（`F-20260825-29`・ひな形未作成） |
| `docs/watch.md`（経過観察） | **未作成** |
| `docs/ops/template.md`（ひな形4部） | **未作成**（F2 の開始時に要る） |

**あと回しが8件増えて、解決が8件。** F1 は「隔離できたことを機械で言えるようにした」
フェーズであって、アプリが良くなったフェーズではない（ルール②③）。

### 次にやること

**フェーズは `F2`（`docs/ops/phase`）。正UI のみで動線を完了させる。**
`docs/ops/plan.md` 第3章が正。着手前に、この順で:

1. **②バッドシナリオ 10個を出してマスター承認を受ける** → `docs/ops/bad-scenarios-F2.md`
   （承認印が無い間、`npm run guard` が `src/` の書き換えを止める）
2. **③再発防止（開始）** → `docs/ops/failure-check-F2-start.md`
3. `docs/ops/template.md`（ひな形4部）を作る — `F-20260825-29` の Guardrail
4. **「＋ 新規カルテを作成する」の復元** — モックに在り現画面に無い（マスター4回指摘の入口）。
   実体 `App.createNewKarte()` は `src/js/ui.js:127` に**在る。押す場所だけが無い**
5. **463px 問題** — 上記のとおり**マスターに2択を確認してから**。私が新デザインを発案しない
6. 合否は `npm run walk` / `npm run walk mistakes` の**画像だけ**で、D-14 の2問に答える

**マスター判断待ち**: 463px の直し方2択 ／ 担当トリマーの既定文2か所（`src/index.html:1953` と `:2076`。
トリマーが何も書かなくても飼い主の画面に長文が出る）／ `photo-trim-action.jpg` の出所 ／
本番のダミー犬3頭と孤児 owner ／ **資格情報3種のローテーション（実顧客データ投入前に必須）** ／
Konva 4カンバスの出力形式。
