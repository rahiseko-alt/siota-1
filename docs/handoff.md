...
## 0. 最新（2026-08-25）— **F1 と F2 が完了。次は F3（正UI とバックエンドをつなぐ）**

**F2 の合否は、マスターが実機で操作して「合格」と判定した**（`docs/decisions.md` D-20260825-40）。
D-14 の2問がいずれも ○ ——最後まで到達できた／間違えても2タッチ以内に戻れた。
...
| `node scripts/guard/solved.mjs F1` / `F2` | **0** | 該当分すべてが3出力で裏づけ済み |

`npm run verify:*` と `npm run preview` は**存在しない**。**F3 で作り直す**（`docs/deferred.md` #8）。

### マスターが自分で動かす手順（Windows で動作確認済み）

```
git clone -b claude/checkin-ert4y4 https://github.com/rahiseko-alt/siota-1.git
cd siota-1
npm ci
npm run build
npm run serve
...
スマホの実機で見るときは `UI_HOST=0.0.0.0` を付ける（PowerShell なら `$env:UI_HOST="0.0.0.0"`）。
起動時にパソコン用とスマホ用の URL を両方表示する。

### F1 で直したもの（8件・3出力あり）

`npm run check` に隔離検査が0本だった／検査が `index.html` を走査していなかった／赤を一度も見ていなかった／
「UI 以外が無い」の基準が無かった／`magazine-view.js` を消したまま呼び出し元が生きていた／
`npm test` が backend を見ず**壊れていても緑**だった／設定が実在しない場所を指し**黙って合格**していた／
まっさらな環境で意味不明に落ちた。詳細は `docs/ops/solved-F1.md`。

### F2 で直したもの（7件・3出力あり）

| # | 何が起きていたか | どう直したか |
|---|---|---|
| **11** | **検査が「カルテを書いた」と言いながら1文字も入力していなかった** | 実在する `#editor-trimmer-letter` に書き、**値を読み返して照合**。握りつぶさない |
| **463px** | ④から⑤へ進めない | `1fr` → `minmax(0, 1fr)`。**文字も余白も変えていない**（`D-20260825-39`） |
| 1 | 新しい犬を作る場所が画面に無い | モックのブロックを**逐語で移した** |
| 4 | 空の `<img>` が**ページURL**を指す | `src=""` を属性ごと削除（11件 → 0件） |
| 5 | ⑤と⑥の区別が絵からつかない | コマのラベルを「同一画面 screen-4」と明記 |
| 6 | 2タッチの数え方 | 実は定義済み。#11 を直して**検査が成立**した |
| 7 | `src` を直しても画面が古い | `npm run walk` は既に `build` を含んでいた |

**#2（戻る導線）は「保留」。** 意匠モックの `screen-3` にも戻るボタンが無く `src` と完全一致だったので、
足すのは「自分の発案を混ぜない」に反する。**`01〜04` タブを撤去しないこと**で担保（`docs/deferred.md` #2）。

### このセッションで自分が出した失敗（重要）

- **`F-20260825-32`**: 「見落とし10個」を**根拠を確かめずに提案した**。マスターの「根拠を持っているのか」
  という指摘で全件実測 → **3件が該当せず**、しかも**最も重い11個目が提案に入っていなかった**
- **`F-20260825-33`**: 「これで試せます」と案内した手順が **Windows で無反応**だった。
  `import.meta.url === \`file://${process.argv[1]}\`` が Windows のパス形式で成立しない。
  `scripts/guard/*.mjs` の直接実行も全部無反応だった（`npm run guard` だけ別経路で動いていて気づけなかった）
- **`F-20260825-31`**: `solved.mjs` が `m` フラグ下の `$` のせいで**本文を1行目までしか読めず**、
  正しく書いた記録8件を全部「書式違反」と報告した

**共通の型は「自分が動かせる環境でしか確かめていない」。**

### 対の指標

| | 件数 |
|---|---|
| **解決**した（3出力あり） | **15 件**（F1 8 / F2 7） |
| `docs/deferred.md` の**あと回し** | **14 件** |
| `docs/failures.md` | 全 **34件** / OPEN **1件** |
| `docs/watch.md` / `docs/ops/template.md` | **どちらも未作成** |

### 次にやること — F3（フェーズは `docs/ops/phase` = `F3`）

> **2026-08-25 チェックアウト時点。** マスターの指示により F3 は**次セッションに回した**。
> このセッションでは F3 に一切着手していない（`src/` も `backend/` も無変更）。

**作業場は閉じている**（`node scripts/guard/gate.mjs src/js/ui.js` = EXIT 1）。
`src/` を触るには先に、この順で:

1. **②バッドシナリオ10個を出してマスター承認** → `docs/ops/bad-scenarios-F3.md`
   **提案の時点で各項に実行出力を付けること**（`F-20260825-32` の教訓）。
   前回は「確かめ方」だけ書いて確かめずに承認を求め、マスターに
   「お前が根拠を持っているのか？」と問われた。実測したら**3件は該当せず**、
   しかも**最も重い1件が提案に入っていなかった**
2. **③再発防止（開始）** → `docs/ops/failure-check-F3-start.md`
   根拠は**見たファイル名か、実行した命令とその出力**にする

**F3 の着手条件は満たしている**（F2 の2問が両方 ○・`D-20260825-40`）。

そのうえで `docs/ops/plan.md` 第4章の結線表どおりに繋ぐ。**F3 で最初に見るべきもの**:

| # | 何が起こり得るか | 根拠 |
|---|---|---|
| 1 | **④で書いた内容が⑥に届かない** | `deferred` #12。実測済み（④の文章 / ⑥は既定文）。**D-12「押せた ではなく 同じ値で届いた で見る」** |
| 2 | **飼い主に届く中身を見る検査が1本も無い** | `deferred` #8。`verify:*` 7本が削除済み。復元元は `git show 6685df5^:scripts/verify-*.mjs` |
| 3 | **削除は Storage → DB の順** | `deferred` #10・`D-20260824-34`。逆にすると写真が誰からも取れなくなる |
| 4 | **`finalize_report` の `null` を成功として扱う** | `AGENTS.md` D-2 |
| 5 | **SQL を一度も実行せずに置く** | `deferred` #9。`verify:migrations` が `npm test` から呼ばれていない |

**マスター判断待ち**: `01〜04` タブの撤去可否（`deferred` #2・**撤去すると戻る手段が消える**）／
スタンプ帯の横スクロール（#11）／担当トリマーの既定文2か所と新規カルテの既定文（#13）／
写真が無い箇所の `alt` 表示（#14）／`photo-trim-action.jpg` の出所／本番のダミー犬3頭と孤児 owner／
**資格情報3種のローテーション（実顧客データ投入前に必須）**／Konva 4カンバスの出力形式。
