...
| 2 | `xss` | 復元済み | 保存されたカルテが飼い主のブラウザで**実行されない**こと（D-11） | `/my/pets/:pet/reports/:report` のみ | — CI で走る。合格条件は書き直した（下記） |
| 3 | `report-roundtrip` | 復元済み | 書いたものが飼い主に**同じ値で届く**か（D-12・最重要） | `/edit` → `/my` | — 4-1 の結線と一緒に書き直した。旧版の目印は1つも使えないので**復元ではなく書き直し** |
| 4 | `delete` | 復元済み | 削除したら写真が**実体として**消えること。`service_role` で数える | `/edit` | 4-1 の結線 ＋ 削除導線 |
| 5 | `draft` | 復元済み | 記入が黙って消えないこと（下書き・復帰・確定後は書けない） | `/edit` | 4-1 の結線 ＋ 下書き保持 |
| 6 | `empty-pet` | 復元済み | カルテ0件の犬に**存在しない履歴**を見せていないこと（D-10） | `/edit` → `/my` | 4-1 の結線（一覧・新規作成） |
| 7 | `invitation` | 復元済み | 新規のお客様が自分のカルテを見られるまで（`claim_invitation`） | `/edit` | 4-1 の結線 ＋ 招待発行の導線 |
| 8 | `m6` | 復元済み | 動線①〜⑥が一気通貫で通ること | `/edit` → `/my` | 4-1 の結線 全部 |
| 9 | `screens` | 復元済み | 各画面に**在るべきボタンが在る**か（手順の外を見る唯一の検査） | `/` `/my` `/edit` | 4-1 の結線 全部 |
