...
- **「次回のおすすめご来店時期」**: **残す。**私の指摘が誤り。`D-20260825-38` で記録し、`D-20260823-13` を一部撤回した。以後これを違反として数えない
- **「担当者の既定文が2か所」**: 書き方が意味不明だったので、実物を見て言い直した（下記）

### 「担当者の既定文」とは、具体的に何か

`src/index.html` の2か所に、**同じ長文が最初から入っている**。

| 場所 | 誰が見るか | 入っている文 |
|---|---|---|
| `1953` カルテ入力画面 ⑦担当トリマーからのメッセージ の入力欄 | トリマー | 「今月もとってもお利口にトリミングさせてくれました。腰のマッサージ中はずっとウトウトして…また来月お会いできるのを楽しみにしております。」 |
| `2076` 顧客カルテ画面 担当トリマーからのメッセージ | **飼い主** | （同じ文） |

- **人間**: トリマーが**何も書かなくても**、飼い主の画面に「今月もとってもお利口に…」と出る。書いていないことが、書いたことになる
- **AI**: `<textarea id="editor-trimmer-letter">` と `<p id="mag-letter-content">` に初期値が直書きされている。意匠モックの見本文がそのまま残ったもの

**マスター判断待ち。** 意匠モックが正である以上、これが「見本として残す」ものなら残す。
ルール④に従い、F1〜F2 の期間中は触らない。
