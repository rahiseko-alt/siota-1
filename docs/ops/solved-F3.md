...
> **この出力は `76b54ad` の CI で実際に見たもの**（16項目）。ここで
> 「招待（QR）を発行する入口が無い」を1件見つけ、それを受けて `#17` の入口を戻した。
>
> **入口を戻したことで、この検査は3項目増えている**（`13c` / `17` / `18`）。
> 増やした分の緑はまだ CI で見ていないので、ここには書かない。
> 見てから貼り直す。**見ていない結果を書かない**（`D-18` 偽-6）。
