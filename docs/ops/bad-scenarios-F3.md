...
### 14. 削除したのに写真が実体として残る（`verify:delete` が無い） — 結果: 該当した ／ 解決済み
...
### 18. 動線①〜⑥が、実データでは途中で切れる（`verify:m6` が無い） — 結果: 該当した ／ 解決済み
- **人間**: F3 の完了条件そのもの。①〜⑥を実データで通しで確かめる検査が無い。
- **AI**: F2 の2問を実データで見る一気通貫。要 4-1 結線 全部。

### 19. 押すべきボタンが画面から消えても、誰も気づかない（`verify:screens` が無い） — 結果: 該当した ／ 解決済み
